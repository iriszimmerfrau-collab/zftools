...
if (typeof window === "undefined") {
  ...
  (() => {
    ...
    if (window.crossOriginIsolated || reloadedByCOI === "coepdegrade") return;
    if (window.isSecureContext && !coepDegrading) {
      navigator.serviceWorker
        .register(window.document.currentScript.src)
        .then(
          (registration) => {
            if (registration.active && !navigator.serviceWorker.controller) {
              window.sessionStorage.setItem("coiReloadedByCOI", "pendingReload");
